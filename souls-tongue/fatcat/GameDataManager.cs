	...
	public static class GameDataManager
	{
		...
		public static bool InitializeFromBND(string bndPath)
		{
			...
			if (BND3.Is(bndPath))
				...
				System.Console.WriteLine("bnd path broke!");
			...
			foreach (var f in bnd.Files)
			{
				...
				else
				{
					...
					System.Console.WriteLine("failed to figure out type from bnd!");
				}
			}
		...
		public static (string Path, System.Numerics.Vector2 UVScale) LookupMTDTexture(string mtdName, string texType)
		{
			//LoadSystex();
			...
			if (MtdDict.ContainsKey(mtdShortName))
			{
				...
				foreach (var x in mtd.Textures)
				{
					...
					{
						System.Numerics.Vector2 uvScale = System.Numerics.Vector2.One;
						if (x.UnkFloats.Count == 2 && x.UnkFloats[0] > 0 && x.UnkFloats[1] > 0)
						{
							uvScale = new System.Numerics.Vector2(x.UnkFloats[0], x.UnkFloats[1]);
						}
			...
			return (null, System.Numerics.Vector2.One);
		}
		...
		/*public static Model LoadObject(string id)
		{
			...
			/*LoadingTaskMan.DoLoadingTaskSynchronous($"LOAD_OBJ_{id}", $"Loading object {id}...", progress =>
			{
				...
				if (GameType == SoulsGames.DS3 || GameType == SoulsGames.BB || GameType == SoulsGames.SDT)
				{
					...
					obj = new Model(null, id, chrbnd, 0, null, null);
				}
		...
		/*public static Model LoadCharacter(string id,
			...
			List<Action> doImportActionList = null)
		{
			Model chr = null;
